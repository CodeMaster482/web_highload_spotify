...
- В системе рекомендаций в рамках сессии на пользователя 
  будем заполнять фильтр блума (**Bloom Filter**) для прослушаных треков,
  что бы в 1 сеансе прослушивания музики не было повторений 
...
![Scheme](./Копия%20Scheme_SpotifyPravkiLastMB.drawio(1).svg)
...
| Сервис | Google Cloud Storage | Конфигурация | CPU | Count | Аренда(\$) в месяц |
|--------|:--------------:|--------------|-----|-------|------------|
| Auth Redis   | + | Intel Ice lake 4x12Core/RAM 4x192GB/800GB | 48 | 4 | $2,500 |
| Auth Redis   | + | Intel Ice lake 4x60Core/RAM 4x192GB/800GB | 60 | 4 | $3,200 |
| Clickhouse | + | Intel Ice lake 2x32Core/RAM 2x128GB/2048GB | 64 | 2 | $1800 |
| Kafka | + | Intel Ice lake 16x16Core/RAM 16x64GB/1024GB | 256 | 16 | $6300 |  
| Envoy | + | 6430/16GB/1xNVMe128Gb/2*10Gb/s | 32 | 2 | $2800 |
| Cassandra | + | 8*6430/8x64GB/2xNVMe2TB/2*10Gb/s | 256 | 8 | $8000 |
| S3 | + | 4*6430/4x64GB/4xNVMe2TB/2*10Gb/s | 128 | 4 | $2000 |
| CDN | - | - | - | 120 | $2000 |

### Покупка собственного оборудования

Если же покупать самому воспользовавшмись конфигураторм

***2U/6338/8x32GB/4xNVMe8TB/10Gb/s*** ~ `$14,600`

1(Auth) + 4(Cassandra) + 2(ClickHouse) + 4(Envoy) + 2(S3) = `$175200`

Для учета реплик и других инстансов системы получим ~ `$350400`

Стоимость аренды в месяц получается 
```
2500 + 3200 + 1800 + 6300 + 2800 + 8000 + 2000 + 2000 = $28600

28600 * 12 = $343200 // в год
```

### Амортизация

Амортизация аренды на `5 лет`

```
343200 * 5 = $1,716,000
```

Амортизация покупки оборудования на `5 лет`

По статистике, за год службы из строя выходит `0,66%` **HDD** отказывает и ни одного **SSD**.

За второй год `1,54%` **HDD** отказали и `0,85%` **SSD**. 

За третий год, **HDD** - `1,38%` и **SSD** - `0,79%`

За чтевертый год, **HDD** - `1,83%` и **SSD** - `1,05%`

За пятый год, **HDD** - `3,55%` и **SSD** - `0,92%`

По скольку у нас используется в основном **SSD**

`26 * 4 = 104` - Кол-во **SSD**

```
104 * (0.66 + 0.85 + 0.79 + 1.05 + 0.92)/100 = 4,4 ~ 5
```
За пять лет в 1 ДЦ будет отказывать 5 **SSD**

Стоимость одного **SSD** составляет `$1,300`

```
$1,300 * 5 = $6,500
```

Заложим на обслуживание 1-ого сервера `$300` в год

```
$300 * 26 * 5 = $39,000
```

Получим амортизацию за 5 лет `$45,500`

### Выводы

Для сервиса нашего маштаба намного выгоднее будет закупить собственные сервера, чем арендовать у облачных провайдеров.
Такая разница начинает ощущаться ~ `через 1 год`

Но это не значит, что нужно отказыватья от Облачных сервисов. Можно пользоваться, например, услугами CDN и хранилищ медиа файлов, тк можно будет гораздо удобнее горизонтально маштабироваться, и не волноваться о синхронизации, консистенции данных в них данных 
